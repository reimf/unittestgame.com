...
test.describe('mt submit insufficient unit test', () => {
    ...
    test('has high scores panel', async({ page }) => {
        ...
        await expect(highScores).toContainText('Level 2 - Mutation Testing - Even Odd: 100%')
    })

    test('has level menu message', async({ page }) => {
        const messages = page.getByTestId('messages')
        const button = messages.getByRole('button')
        await expect(button).toHaveText('I want to play Level 3 - Test Driven Development - Fizz Buzz')
    })
})
