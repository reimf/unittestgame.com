...
import { Discount } from '../../src/level-discount.js'
import { JavaScript } from '../../src/programming-language-javascript.js'

test.describe('class Discount', () => {
    const conversationLanguage = new English()
    const programmingLanguage = new JavaScript()
    const level = new Discount(conversationLanguage, programmingLanguage, new FixedPicker(), new MapStore(), 4)

    test('has the correct description', () => {
        expect(level.description()).toBe('Level 4 - Discount')
    })
    ...
    test('has the correct amount of candidates', () => {
        expect(level.candidates).toHaveLength(243)
    })
    ...
    test('has the correct amount of perfect candidates', () => {
        expect(level.perfectCandidates).toHaveLength(4)
    })
