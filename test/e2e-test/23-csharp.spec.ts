...
test.describe('csharp programming language', () => {
    test('has the simplest candidate rendered in C# format in the current function panel', async ({ page }) => {
        await page.goto('/game?speed=fast&picker=fixed&programming_language=csharp')
    ...
    test('has a plain bool return type in the current function panel', async ({ page }) => {
        await page.goto('/game?speed=fast&programming_language=csharp&setitem=level-battery-level-finished:1')
