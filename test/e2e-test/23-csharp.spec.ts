...
test.describe('csharp programming language', () => {
    test('has the simplest candidate rendered in C# format in the current function panel', async ({ page }) => {
        await page.goto('/game.html?speed=fast&picker=fixed&programming_language=csharp')
        ...
        await expect(codeLines).toContainText('static string powerMode(int batteryLevel){    return "UNKNOWN";}')
    })

    test('has a plain bool return type in the current function panel', async ({ page }) => {
        await page.goto('/game.html?speed=fast&programming_language=csharp&setitem=level-battery-level-finished:1')
