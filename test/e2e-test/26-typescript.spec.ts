...
test.describe('typescript programming language', () => {
    test('has the simplest candidate rendered in TypeScript format in the current function panel', async ({ page }) => {
        await page.goto('/game?speed=fast&picker=fixed&programming_language=typescript')
        ...
        await expect(codeLines).toContainText('function powerMode(batteryLevel: number): string {    return "UNKNOWN"}')
    })

    test('has a plain boolean return type in the current function panel', async ({ page }) => {
        await page.goto('/game?speed=fast&programming_language=typescript&setitem=level-battery-level-finished:1')
