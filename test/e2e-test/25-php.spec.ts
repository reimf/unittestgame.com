...
test.describe('php programming language', () => {
    test('has the simplest candidate rendered in PHP format in the current function panel', async ({ page }) => {
        await page.goto('/game.html?speed=fast&picker=fixed&programming_language=php')
        ...
        await expect(codeLines).toContainText('function powerMode(int $batteryLevel): string {    return "UNKNOWN";}')
    })

    test('has a bool return type in the current function panel', async ({ page }) => {
        await page.goto('/game.html?speed=fast&programming_language=php&setitem=level-battery-level-finished:1')
